<template>
    <div>
		...
		<b-alert variant="info" :show="!!selectedOrder" class="wpo-add-customer-modal__order-customer">
		    {{ selectedOrderMessage }} <a :href="selectedOrder && selectedOrder.loaded_order_url" target="_blank">#{{ selectedOrder && selectedOrder.loaded_order_number }}</a>
		</b-alert>
	    </span>
		...
		<b-container>
		    <slot name="add-customer-address-modal-header"></slot>
		</b-container>
				...
				v-show="!!group.fields.length"
                            ...
                            <b-row>
                                <b-col cols="12" md="6" v-for="field in group.fields" :key="field.key">
                                    <span>
                                        <label class="mr-sm-2" :class="[ emptyRequiredFieldsList.indexOf(field.key) > -1 ? 'text-danger' : '' ]">{{ requiredFieldsList.indexOf(field.key) > -1 ? field.label + ' *' : field.label }}</label>
	...
	export default {
		...
		methods: {
			...
			isCustomerValid() {
			    ...
			    var valid = true;
			    var modal = typeof this.$children[0] !== 'undefined' ? this.$children[0] : [];
				...
				if (typeof child.checkCustomer !== 'undefined') {
				    if (!!!child.checkCustomer()) {
					valid = false;
					return;
				    }
				}

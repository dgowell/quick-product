    ...
    <div id="postbox-container-2" class="postbox-container">
		...
		<div class="wpo-search-options-block" :class="{'wpo-show-search-options': showProductSearchOptions}">
			...
			<div class="wpo-find-product-search-block">
			    ...
			    @input="isAllowConfigureProduct ? (useConfigureProductActionAsDefault ? configureProduct() : null) : addProductItemsToCart([{product: product, qty: 1}])"
				...
				<label class="option__element__full-screen-mode">
					...
					<span class="option__input_price" v-if="isShowPriceInputInAdvancedSearch">
					    <input type="text" autocomplete="off" placeholder="0" v-model="multipleSelectedProducts[props.option.value]['item_cost']" size="4" v-bind:disabled="!cartEnabled" @mousedown="multipleSelectedProducts[props.option.value]['checked'] = true" class="price">
					</span>
		    ...
		    <div class="col-12 col-md-7">
			    ...
			    <tr v-if="!hideAddShipping" v-for="(shippingPackage, index) in shipping.packages">
				    <td class="label-total">
					    <a href="#" @click.prevent.stop="cartEnabled && !selectOptimalShipping ? addShipping(shippingPackage.hash) : null"
			    ...
			    <tr v-if="giftCardEnabled" class="gift-card-list-item" v-for="(card, index) in giftCardList">
                                <td class="label-total">{{ giftCardLabel }}: <span v-html="card.title"></span></td>
                                <td width="1%"></td>
                                <td>
					...
					<span v-if="card.amount" v-html="wcPrice(card.amount)"></span>
                            ...
                            <tr v-if="giftCardEnabled && !hideAddGiftCard" class="gift-card-list-add">
                                <td class="label-total">
                                    <a href="#" @click.prevent.stop="cartEnabled ? addGiftCard() : null"
                                       :class="{disabled: !cartEnabled}">
                                        {{ addGiftCardLabel }}
                                    </a>
                                </td>
                            ...
                            <tr v-if="orderTotalCustomHtml">
                                <td colspan="4"><div class="wpo-total-custom-html" v-html="orderTotalCustomHtml"></div></td>
                            </tr>
                        ...
                        <tr>
                            <td>
						     ...
						     class="copy-link-block"
                                            >
						<b-input-group>
						    <b-form-input size="sm" :value="copyLinkUrl" :readonly="true" ref="copyLinkInput"></b-form-input>
    ...
    export default {
		...
		var requestSearchProduct = () => {
			...
			if (this.isBarcodeMode && products.length < 2) {
			    this.addProductItemsToCart([{product: products[0], qty: 1, item_cost: product[0].item_cost}]);
			}
                    });
                }
...
//                    });
		...
		if (cart) {
			...
			this.showAlert();
			this.errorMessage = cart.coupon_errors.map(function (error) { return error.notice; }).join(', ');
		    }
        ...
        for (var product_id in this.multipleSelectedProducts) {
          if (this.multipleSelectedProducts[product_id].checked) {
            items.push({
              ...
              item_cost: this.isShowPriceInputInAdvancedSearch ? this.multipleSelectedProducts[product_id].item_cost : null
            })
          }
        }
	    ...
	    copyLink() {
		this.$copyText(this.copyLinkUrl).then((e) => {

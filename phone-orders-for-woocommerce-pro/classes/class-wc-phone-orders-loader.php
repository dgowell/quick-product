...
class WC_Phone_Orders_Loader {
	...
	public static function check_user_capability() {
		...
		if ($column === 'order_number' && get_post_meta( $post_id, WC_Phone_Orders_Loader::$meta_key_order_creator, true ) ) {
	...
	public static function admin_footer_text( $footer_text ) {
		...
		if ( isset( $current_screen->id ) && apply_filters( 'wpo_display_admin_footer_text', in_array( $current_screen->id, $pages, true ) ) ) {
			// Change the footer text.
			if ( ! get_option( 'phone-orders-for-woocommerce-rated' ) ) {
				$footer_text = sprintf(
					/* translators: 1: WooCommerce 2:: five stars */
					__( 'If you like %1$s please leave us a %2$s rating. Thank you so much in advance!', 'phone-orders-for-woocommerce' ),
					sprintf( '<strong>%s</strong>', esc_html__( 'Phone Orders for WooCommerce', 'phone-orders-for-woocommerce' ) ),
					'<a href="https://wordpress.org/support/plugin/phone-orders-for-woocommerce/reviews?rate=5#new-post" target="_blank" class="wpo-rating-link" aria-label="' . esc_attr__( 'five star', 'phone-orders-for-woocommerce' ) . '" data-rated="' . esc_attr__( 'Thanks :)', 'phone-orders-for-woocommerce' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
				wc_enqueue_js(
	...
	public function add_billing_phone_email_to_wpo_customer_formatted_address() {
		add_filter( 'wpo_customer_formatted_address', function ( $fields, $customer_data, $type ) {
			if ( ( $type == 'billing' ) && isset( $customer_data['billing_phone'] ) ) {
				$fields['wpo_billing_phone'] = $customer_data['billing_phone'];
			}
			if ( ( $type == 'billing' ) && isset( $customer_data['billing_email'] ) ) {
				$fields['wpo_billing_email'] = $customer_data['billing_email'];
			}
	...
	public function add_action_links( $links ) {
		$mylinks = array(
			'<a href="' . admin_url( 'admin.php?page=phone-orders-for-woocommerce' ) . '">' . __( 'Create Order',
				'phone-orders-for-woocommerce' ) . '</a>',
			'<a href="https://docs.algolplus.com/phone-order-for-woocommerce/" target="_blank">' . __( 'Docs',
				'phone-orders-for-woocommerce' ) . '</a>',

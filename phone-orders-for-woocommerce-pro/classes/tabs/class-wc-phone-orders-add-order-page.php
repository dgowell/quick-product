	...
	public function render() {
		...
		$this->tab_data = array(
                    ...
                    'editAddressModalSettings'   => array(
                        ...
                        'customerAddressAdditionalKeys'	     => $this->customer_address_additional_keys,
                        'noOptionsTitle'		     => __( 'List is empty.', 'phone-orders-for-woocommerce' ),
                        'personalFieldsOrder'		     => apply_filters('wpo_edit_address_modal_personal_fields_order', array('email', 'role', 'first_name', 'last_name', 'company', 'phone', 'locale')),
		...
		$tab_data = array(
			'findOrCreateCustomerSettings' => array(
				...
				'customerAddressAdditionalKeys'   => $this->customer_address_additional_keys,
				'multiSelectSearchDelay'          => $this->multiselect_search_delay,
				'noOptionsTitle'                  => __( 'Type search phrase to see results', 'phone-orders-for-woocommerce' ),
	...
	protected function ajax_load_items( $request ) {
		...
		foreach ($request['items'] as $item) {
		    ...
		    if ($product->is_type( 'grouped' )) {
                        $ids = array_merge($ids, array_map(function($_item) use ($item) { return array('id' => $_item, 'qty' => $item['qty'], 'item_cost' => isset($item['item_cost']) ? $item['item_cost'] : ''); }, $product->get_children()));
		    } else {
			$ids[] = array('id' => $item['id'], 'qty' => $item['qty'], 'item_cost' => isset($item['item_cost']) ? $item['item_cost'] : '');
		    }
		}
...
//				}
		...
		foreach ( $products as $product_id => $product) {
			...
			$result[] = apply_filters( 'wpo_search_products_result_item', array(
				...
				'add_to_exclude'	=> !$product->is_type( "grouped" ),
                                'qty_step'              => apply_filters( 'woocommerce_quantity_input_step', $this->option_handler->get_option( 'allow_to_input_fractional_qty' ) ? '0.01' : '1', $product ),
                                'min_qty'               => apply_filters( 'woocommerce_quantity_input_min', $this->option_handler->get_option( 'allow_to_input_fractional_qty' ) ? '0.01' : '1', $product ),
	...
	protected function get_is_valid_product( $product ) {
		...
		if ( ! $product->is_purchasable() AND $product->is_type( 'variation' ) AND ! $option_handler->get_option( 'sell_disable_variation' ) ) {
			return false;
		}
	...
	if ( $customer_id ) {
        ...
        $state = array(
            ...
            'default_items'                     => $this->get_formatted_product_items_by_ids(array_map(
                function ($item) {

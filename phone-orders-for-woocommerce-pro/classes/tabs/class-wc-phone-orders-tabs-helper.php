		...
		if ( ! empty( $result['shipping']['packages'] ) ) {
			foreach ( $result['shipping']['packages'] as $package_key => $package ) {
				if ( isset( $package['chosen_rate'] ) ) {
					$total = $package['chosen_rate']['label'] . " : " . self::format_shipping_total( $package['chosen_rate']['cost'], $package['chosen_rate']['tax'] );
				} else {
					$total = __( 'Not selected!', 'phone-orders-for-woocommerce' );
				}

				$row .= sprintf( "%s %s (%s)<br />", __( "Package ", 'phone-orders-for-woocommerce' ), $package_key + 1,
					$total );
			}
		}

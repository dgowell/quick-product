<template>
    <div class="col-12" v-show="show" v-if="Object.keys(fieldList).length !== 0">
        <hr>
        <strong v-if="customFieldsLabelOption"><label class="mr-sm-2">{{ customFieldsLabelOption }}</label></strong>
        <custom-fields
                v-bind:id="'customer_custom_fields'"
                v-bind:storedFields="fields"
                v-bind:fieldListRows="fieldListRows"
                v-bind:dateFormat="dateFormat"
                v-bind:singularClassName="'customer-modal-footer__custom_field col-12 col-md-6'"
...
<script>
	...
	export default {
		...
		created: function () {
			...
			this.$root.bus.$on('customer-custom-fields-set-custom-fields', (data) => {
		...
		computed: {
            ...
            customFieldsLabelOption() {
                return this.getSettingsOption( 'customer_custom_fields_header_text' );
            },
		},
		watch: {
